...
// Neither MSVC nor Intel support enough of C++14 yet (in particular, as of MSVC
// 2015 and ICC 17 beta, neither support extended constexpr, which we rely on in
// descr.h), so don't enable pybind CPP14 features for them.
...
#define PYBIND11_LONG_AS_UNSIGNED_LONGLONG(o)                                  \
  (PyInt_Check(o) ? (unsigned long long)PyLong_AsUnsignedLong(o)               \
...
#define PYBIND11_INTERNALS_ID                                                  \
  "__pybind11_" PYBIND11_TOSTRING(                                             \
      PYBIND11_VERSION_MAJOR) "_" PYBIND11_TOSTRING(PYBIND11_VERSION_MINOR) "__"
...
struct buffer_info {
  ...
  std::vector<size_t> strides; // Number of entries between adjacent entries
  ...
  explicit buffer_info(Py_buffer *view, bool ownview = true)
      : ptr(view->buf), itemsize((size_t)view->itemsize), size(1),
        format(view->format), ndim((size_t)view->ndim),
        shape((size_t)view->ndim), strides((size_t)view->ndim), view(view),
        ownview(ownview) {
...
/// Core part of the 'instance' type which POD (needed to be able to use
/// 'offsetof')
template <typename type> struct instance_essentials {
  PyObject_HEAD type *value;
  PyObject *weakrefs;
  bool owned : 1;
  bool holder_constructed : 1;
};

/// PyObject wrapper around generic types, includes a special holder type that
...
struct internals {
  std::unordered_map<std::type_index, void *>
      registered_types_cpp; // std::type_index -> type_info
  std::unordered_map<const void *, void *>
      registered_types_py; // PyTypeObject* -> type_info
  std::unordered_multimap<const void *, void *>
      registered_instances; // void * -> PyObject*
  ...
  std::unordered_map<size_t, PyObject *>
      bases; // one base type per `instance_size` (very few)
...
// Extracts the first type from the template parameter pack matching the
// predicate, or Default if none match.
...
struct first_of<Predicate, Default, T, Ts...> {
  using type = typename std::conditional<
      Predicate<T>::value, T,
      typename first_of<Predicate, Default, Ts...>::type>::type;
};
template <template <class> class Predicate, class Default, class... T>
using first_of_t = typename first_of<Predicate, Default, T...>::type;
...
using is_template_base_of =
    decltype(is_template_base_of_impl<Base>::check((remove_cv_t<T> *)nullptr));
#else // MSVC2015 has trouble with decltype in template aliases
struct is_template_base_of : decltype(is_template_base_of_impl<Base>::check(
                                 (remove_cv_t<T> *)nullptr)) {
};
#endif
...
public:
  ...
  error_already_set(error_already_set &&e)
      : std::runtime_error(e.what()), type(e.type), value(e.value),
        trace(e.trace) {
    e.type = e.value = e.trace = nullptr;
  }
  ...
  /// Clear the held Python error state (the C++ `what()` message remains
  /// intact)
  void clear() {
    restore();
    PyErr_Clear();
...
struct format_descriptor<
    T, detail::enable_if_t<detail::is_fmt_numeric<T>::value>> {
  static constexpr const char c =
      "?bBhHiIqQfdgFDG"[detail::is_fmt_numeric<T>::index];
